...
def api_creer_client(request):
    try:
        ...
        client_data = payload.get('client', payload)

        # 🛠️ BYPASS DE LA SÉCURITÉ POUR LES TESTS
        user_pour_test = request.user
        
        if not user_pour_test.is_authenticated:
            # On récupère le tout premier utilisateur de votre base de données (souvent le superuser id=1)
            user_pour_test = User.objects.first()
            
            # Si la base de données est totalement vide, on en crée un à la volée pour le test
        ...
        # On appelle votre fonction avec l'utilisateur authentifié ou l'utilisateur de test
        client = creer_client(client_data, user_pour_test)
        ...
        return JsonResponse({"erreur": f"Erreur lors des tests : {str(e)}"}, status=500)

    return JsonResponse(
        {"message": "Client créé avec succès (Mode Test).", "client": client_vers_dict(client)},
        status=201,
    ) """

""" Contient la securite USER --> LOGIN
@require_POST
@transaction.atomic
def api_creer_client(request):
    try:
        ...
        if 'dossier' in data:
            ...
            dossier = Dossier.objects.create(
                client=client, avocat_referent=request.user,
        ...
        if 'facture' in data and dossier:
            f = data['facture']
        ...
        return JsonResponse({'message': 'Client créé avec succès.', 'redirect': '/clients/'})
    except Exception as e:
    ...
    try:
        with transaction.atomic():
            ...
            if dossier and dossier.id:
                ...
                # Sous-bloc : Facture (Ajusté selon votre modèle réel)
                if 'facture' in data:
                    #f = data['facture']
                    
                    # Génération d'un numéro de facture unique automatique pour le test
                    #prefixe_num = timezone.now().strftime("FAC-%Y%m%d-")
                    #numero_unique = f.get('numero') or f"{prefixe_num}{uuid.uuid4().hex[:6].upper()}"
                ...
                # Sous-bloc : Événement Agenda (Ajusté selon votre modèle réel)
                if 'agenda' in data:
                    #a = data['agenda']
                    EvenementAgenda.objects.create(
                        ...
                        titre=a['titre'], 
                        type=a['type'],
                        # Si la date reçue est vide, on met l'heure actuelle pour éviter le crash de l'index
                        date_heure=a.get('date_heure') or timezone.now(), 
                        critique=a.get('critique', False),
                        created_by=user_pour_test
                        # REMARQUE : 'description' a été supprimé ici car votre modèle EvenementAgenda ne le possède pas !
                    )
            else:
                ...
                # --- CONSTRUCTION DU RÉCAPITULATIF OPTIMISÉ (ZÉRO REQUÊTE SUPPLÉMENTAIRE) ---
            ...
            if dossier or facture or agenda:
                #summary += f"<br><b>📂 Dossier :</b> Ref: {str(dossier.id)[:8].upper()} / {dossier.intitule}"
        ...
        #return JsonResponse({'message': 'Client et tous les composants liés créés avec succès.', 'redirect': '/clients/'})
